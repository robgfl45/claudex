...
VERIFY_DISPOSITIONS={"accept-and-correct","already-satisfied"}
VERDICTS={"closed","not_closed","closure_requires_new_review"}
...
def validate_registry(reg, *, repo_root, original_sha):
 ...
 if reg["repo_root"]!=repo_root or reg["snapshot_sha256"]!=original_sha: raise ValueError("registry repository/original snapshot identity mismatch")
 _text(reg["review_id"],"review_id"); _text(reg["topic"],"topic")
 if not isinstance(reg["findings"],list) or len(reg["findings"])>500: raise ValueError("registry findings invalid")
 ids=[]
 required={"finding_id","persona_id","severity","scope_anchor","underlying_risk","failure_scenario","repository_evidence","proposed_remedy"}
 for f in reg["findings"]:
  if not isinstance(f,dict) or set(f)!=required: raise ValueError("registry finding schema mismatch")
  if not FINDING_RE.fullmatch(f["finding_id"]) or f["severity"] not in {"high","medium","low"}: raise ValueError("registry finding identity/severity invalid")
  ids.append(f["finding_id"])
 if len(ids)!=len(set(ids)): raise ValueError("registry contains duplicate finding IDs")
 return reg

def validate_manifest(man, reg, *, repo_root, original_sha, final_sha, registry_sha, prior_result_sha=None, recheck_ids=None):
 keys={"schema_version","review_id","engine","repo_root","topic","original_snapshot_sha256","final_plan_sha256","registry_sha256","attempt","prior_result_sha256","dispositions"}
 ...
 if attempt==1:
  if man["prior_result_sha256"] is not None or prior_result_sha is not None: raise ValueError("attempt 1 cannot bind prior result")
 else:
  if not isinstance(prior_result_sha,str) or man["prior_result_sha256"]!=prior_result_sha: raise ValueError("attempt 2 must bind the exact prior result digest")
 rows=man["dispositions"]
 if not isinstance(rows,list) or len(rows)>500: raise ValueError("dispositions invalid")
 row_keys={"finding_id","disposition","rationale","changed_sections","approval_reference","non_plan_blocking_justification"}
 byid={}; registry_byid={x["finding_id"]:x for x in reg["findings"]}
 for row in rows:
  if not isinstance(row,dict) or set(row)!=row_keys: raise ValueError("disposition row schema mismatch")
  fid=row["finding_id"]
  if fid in byid: raise ValueError("duplicate disposition finding ID")
  if fid not in registry_byid: raise ValueError("unknown disposition finding ID")
  if row["disposition"] not in DISPOSITIONS: raise ValueError("invalid disposition")
  _text(row["rationale"],"rationale")
  sections=row["changed_sections"]
  ...
  if row["disposition"] not in {"accept-and-correct","already-satisfied"} and sections: raise ValueError("parent-owned disposition changed_sections must be empty")
  ...
  if row["disposition"]=="accept-risk" and not row["approval_reference"]: raise ValueError("accept-risk requires explicit Rob approval_reference")
  finding=registry_byid[fid]
  if row["disposition"]=="defer-to-implementation" and finding["severity"] in {"high","medium"} and not row["non_plan_blocking_justification"]: raise ValueError("high/medium defer requires explicit non-plan-blocking justification")
  byid[fid]=row
 expected_ids=set(registry_byid) if attempt==1 else set(recheck_ids or ())
 if attempt==2 and (not expected_ids or any(byid[x]["disposition"] not in VERIFY_DISPOSITIONS for x in byid)): raise ValueError("attempt 2 must narrowly recheck prior not_closed verification rows")
...
def validate_verifier(obj, expected_id):
 keys={"finding_id","verdict","evidence","reason"}
 if not isinstance(obj,dict) or set(obj)!=keys or obj.get("finding_id")!=expected_id or obj.get("verdict") not in VERDICTS: raise ValueError("verifier schema/ID/verdict mismatch")
 evidence=obj["evidence"]
 if not isinstance(evidence,list) or len(evidence)>MAX_ITEMS: raise ValueError("verifier evidence invalid")
 for item in evidence: _text(item,"verifier evidence")
 _text(obj["reason"],"verifier reason")
 return obj
