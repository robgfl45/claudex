...
4. Run the adapter once with `--preflight-only` before the long delegated review, then run its `sweep-v2` engine with the selected explicit cap. With the checkout adapter, normally omit `--plugin-root`; pin it only for a deliberately reviewed different plugin. The review command must invoke `/claudex:plan --engine sweep-v2 --from-draft --skip-interview --rounds <CAP> ...` through the adapter.
