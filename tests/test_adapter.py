...
""")
    ...
    def test_source_runbook_stages_review_v3_targeted_closure_and_rollback(self):
        ...
        self.assertIn("--engine review-v3 --rounds 1", runbook)
        self.assertIn("--budget-usd 10", runbook)
        self.assertIn("--budget-usd 10.00", adapter_docs)
        self.assertIn("at least 4,200 seconds", runbook)
        self.assertIn("rollback/exceptional legacy", runbook)
        self.assertIn("exactly one frozen broad pass", skill)
        self.assertIn("not the automatic next step", skill)
        self.assertIn("merged-head live proof", skill)
        self.assertIn("attempt-2", runbook)
        self.assertIn("accept-risk", targeted)
        self.assertIn("Rob approval reference", targeted)
        self.assertNotIn("--rounds 5", skill)
        self.assertIn("accepted_after_targeted_closure", targeted)
        self.assertIn("adapter_converged", targeted)
        self.assertIn("Only `accept-and-correct` and `already-satisfied`", targeted)
        self.assertIn("leaf owns the authoritative preflight", skill)
        self.assertIn("must not substitute", runbook)
        self.assertIn("targeted-closure.md", resumed)
        self.assertIn("subscription-backed", skill)
